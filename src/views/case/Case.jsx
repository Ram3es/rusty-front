...
import HistoryDrops from './HistoryDrops'
...
const Case = (props) => {
  ...
  const itemsUpdate = () => {
    socket.emit('rewards:case', { caseId: Number(props.searchParams.id) }, (data) => {
      ...
      if (data.cs) {
        let rundomItems = []
        for (let i = 0; i <= 100; i++) {
          const randomItem = data.cs.items[Math.floor(Math.random() * data.cs.items.length)]
          rundomItems.push({
  ...
  const startGame = () => {
    socket.emit('rewards:case:open', { caseId: Number(props.searchParams.id) }, (data) => {
      if (data.msg) {
        toastr(data)
      }
  ...
  const getItemLiderCard = (rall) => {
    const wonItem =
      rollCase()?.items[rollCase()?.items?.findIndex((item) => item.name === rall.item)]
    return (
      <div
        ...
        >
          ...
          <div class='w-full h-full group-hover:hidden flex justify-start items-center gap-4'>
            ...
            <div class='flex flex-col items-start max-w-full overflow-hidden gap-1'>
              <p class='text-16 text-gray-8c font-semibold truncate max-w-full'>{wonItem.name}</p>
          ...
          <div class='w-full h-full hidden lex group-hover:flex justify-start items-center gap-4'>
              ...
              <div class='w-14 h-14 flex items-center justify-center'>
                <div
                  class={`w-11 h-11 rounded-full relative overflow-hidden p-sm bg-gradient-to-b ${
                    ranksBorderColor[rall.level?.league] ?? ''
                  }`}
                >
                  <img
            ...
            <div class='flex flex-col items-start max-w-full overflow-hidden gap-1'>
              ...
              <div class='flex'>
                <p class='text-14 text-white font-medium font-Oswald'>
                  {dayjs(rall.timestamp).format('MM/DD/YYYY') || ''}
                </p>
              </div>
            </div>
          </div>
                  ...
                  <div class='w-full max-w-md flex flex-col items-center sm:items-start gap-2'>
                    <div>
                      <p class='text-gray-47 text-14 flex' onClick={() => copy(serverSeed())}>
                        Server Seed Hash:
                        <span class='max-w-[5rem] truncate mr-0.5'>{serverSeed()}</span>
                        <svg
                          class='w-4 ml-3'
                          xmlns='http://www.w3.org/2000/svg'
                      ...
                      <p class='text-gray-47 text-14'>Client Seed: {clientSeed()}</p>
            ...
            <div class={`center flex-col gap-1 duration-200 h-full overflow-hidden`}>
              ...
              >
                <g filter='url(#filter0_d_257_526)'>
                  <path
                    d='M72 9.24445L83 9.24445L72 -6.71591e-06L61 9.24432L72 9.24445Z'
                    fill='url(#paint0_linear_257_526)'
                  />
                </g>
                <path d='M143 9.74533L1.66893e-06 9.74533' stroke='url(#paint1_linear_257_526)' />
                <defs>
                  <filter
                    id='filter0_d_257_526'
                    x='57'
                    y='0'
                    width='30'
                    height='17.2444'
                  ...
                  >
                    ...
                    <feBlend
                      mode='normal'
                      in2='BackgroundImageFix'
                      result='effect1_dropShadow_257_526'
                    />
                    <feBlend
                      mode='normal'
                      in='SourceGraphic'
                      in2='effect1_dropShadow_257_526'
                      result='shape'
                    />
                  </filter>
                  <linearGradient
                    id='paint0_linear_257_526'
                    x1='72'
                    y1='22.0399'
                    x2='72'
                    y2='0.000122064'
              ...
              >
                <path
                  d='M0.499084 211L0.499084 -2.98023e-05'
                  stroke='url(#paint0_linear_156_9073)'
                />
                <defs>
                  <linearGradient
                    id='paint0_linear_156_9073'
                    x1='0.499084'
                    y1='4.75222'
                    x2='0.499084'
                    y2='211'
                      ...
                      {(val, index) => (
                        <div class='w-48 h-full center relative' style={{ 'min-width': '7rem' }}>
                          {index() === 87 ? (
                            <img
                              ref={wonItem}
                              class='w-24 h-auto relative z-10 opacity-70 scale-75 transition'
                              alt='jackpot spinner'
                              src={
                                val?.image ||
                            ...
                            <img
                              class='w-24 h-auto relative z-10 opacity-70 scale-75'
                              alt='jackpot spinner'
                              src={
                                val?.image ||
                                'https://avatars.akamai.steamstatic.com/fef49e7fa7e1997310d705b2a6158ff8dc1cdfeb_full.jpg'
          ...
          <div class='w-full flex flex-col gap-3'>
            ...
            <div class='w-full grid grid-cols-potential-drop--item gap-2'>
              <For each={rollCase()?.items || []}>
                {(item) => <PotentialDropItem skin={item} caseName={rollCase()?.name} />}
              </For>
            </div>
          </div>
        </div>
