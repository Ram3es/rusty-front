...
const CaseUnboxing = (props) => {
  ...
  const itemsUpdate = () => {
    socket.emit('case:get', { caseId: Number(props.searchParams.id) }, (data) => {
    ...
    if (rollCase()) {
      socket.on(`cases:recentdrop:${Number(rollCase().id)}`, (data) => {
        ...
        setCaseStatistic((prev) => {
          const arr = { ...prev }
          arr.recentDrops.unshift({ ...data, price: data.item_price, item: data.name })
          if (arr.length > 10) {
            arr.recentDrops.pop()
          }
          if (
            ...
            arr.bestValue = { ...data, price: data.item_price, item: data.name }
          }
          return arr
        })
      })
