...
const CreateCaseBattle = (props) => {
  ...
  const getSelectedCasesCost = () =>
    modeToCreate().cases.reduce(
      (total, c) => (total += c.qty * casesState().find((obj) => obj.id === c.caseId)?.price || 0),
      0
    )
  ...
  const getModeColor = () => {
    return (modeToCreate().mode === 'royal' || modeToCreate().mode === 'team') &&
          ...
          <div class='flex items-center justify-between mb-4'>
            <NavLink href={URL.GAMEMODES.CASE_BATTLES}>
              <div class='flex gap-2 items-center px-3 py-2 border-2 border-white border-opacity-5 rounded-4 drop-shadow w-max'>
                <ArrowBack />
                <span class='font-SpaceGrotesk text-14 text-gray-9a'>Exit Creation</span>
              </div>
            </NavLink>
          </div>
        </div>
                      ...
                      {(_, index) => (
                        <>
                          ...
                          {index() + 1 !== option.qty &&
                            option.mode !== 'group' &&
                            (option.mode === 'royal' ||
                              (option.mode === 'team' && index() !== 0 && index() !== 2)) && (
                ...
                <div class='border border-yellow-ffb/30 rounded-4 w-9 h-9 flex items-center justify-center'>
                  ...
                  >
                    <svg
                      ...
                      xmlns='http://www.w3.org/2000/svg'
                      style={{
                        filter: 'drop-shadow(0px 0px 8px rgba(255, 180, 54, 0.48))'
                      }}
                    >
                      <path
                        fill-rule='evenodd'
            ...
            >
                ...
                <div class='border border-purple-9f/30 rounded-4 w-9 h-9 flex items-center justify-center'>
                  ...
                  >
                    <svg
                      ...
                      xmlns='http://www.w3.org/2000/svg'
                      style={{
                        filter: 'drop-shadow(0px 0px 8px rgba(159, 151, 249, 0.48))'
                      }}
                    >
                      <path
              ...
              <div class='center px-4 py-3 bg-white bg-opacity-[0.01] rounded-r-6 border h-full border-white border-opacity-5'>
                <Toggle
                  ...
                  onChange={(isChecked) =>
                    setModeToCreate((prev) => ({ ...prev, private: isChecked ? 1 : 0 }))
                  }
                  color='purple'
                />
              </div>
        ...
        <div class='center flex flex-col-reverse sm:flex-row gap-4 p-4 xl:px-8 xxl:px-14 llg:max-w-[calc(100vw-324px)] bg-control-panel'>
          ...
          <Show when={modeToCreate().fundBattle}>
            <Dropdown
              ...
              submitItem={(level) => setModeToCreate((prev) => ({ ...prev, minLevel: level }))}
            />
          </Show>
        </div>
      </div>
