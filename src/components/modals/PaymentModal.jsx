...
import { playDeselectItemSound, playSelectItemSound } from '../../utilities/Sounds/ItemsSound'
...
const PaymentModal = (props) => {
  ...
  const updateInventory = () => {
    if (!isItemsRefresh()) {
      socket.emit('steam:inventory', { asc: descending() }, (data) => {
        ...
        if (
          (!data.error && data.data && props.searchParams?.deposit && props.searchParams?.items) ||
          props.pathname() == URL.GAMEMODES.JACKPOT_DEPOSIT
        ) {
  ...
  const cryptomethods = {
    ...
    eth: {
      icon: (
        ...
        >
          ...
          <path d='M7 12.6908L12.5 4L17.9999 12.6908L12.5 15.8511L7 12.6908Z' fill='white' />
          <path d='M12.5 4L18 12.6908L12.5 15.8511V4Z' fill='#CCCCCC' />
          <path d='M7 13.876L12.5 17.0363L17.9999 13.876L12.5 21.3817L7 13.876Z' fill='white' />
          <path
            d='M12.5 17.0364L17.9999 13.8761L12.5 21.3818V17.0364ZM7 12.691L12.5 10.3208L17.9999 12.691L12.5 15.8513L7 12.691Z'
            fill='#CCCCCC'
          />
          <path d='M12.5 10.3208L18 12.691L12.5 15.8513V10.3208Z' fill='#A9A9A9' />
        </svg>
      ),
      name: 'ethereum',
      color: '#6481E7',
    ...
    'erc20/usdc': {
      icon: <img class='w-[20px] h-[20px]' src={UsdcCryptocurrency} alt='UsdcCryptocurrency' />,
      name: 'USDC-ERC',
      color: '#2775CA',
  ...
  const changeDescending = () => {
    ...
    if (props.searchParams?.deposit || props.pathname() == URL.GAMEMODES.JACKPOT_DEPOSIT) {
      updateInventory()
    } else if (props.searchParams?.withdraw) {
          ...
          >
              ...
              <div class='center gap-3 flex-col sm:flex-row'>
                <NavLink href={`${props.pathname()}?deposit=true`}>
                  <div class='flex gap-2 items-center p-3 border-2 border-white border-opacity-5 rounded-4 drop-shadow w-max'>
                    <ArrowBack />
                    <span class='font-SpaceGrotesk text-14 text-gray-9a'>Back to Methods</span>
                  </div>
                </NavLink>
                <div class='flex flex-col'>
                  <div
                    class={`${
                      props.pathname() == URL.GAMEMODES.JACKPOT_DEPOSIT ? 'opacity-0' : ''
                  ...
                  <div class='w-full flex justify-between items-center gap-2'>
                    <p
                      class={`text-20 text-white font-bold font-SpaceGrotesk uppercase truncate ${
                        props.pathname() == URL.GAMEMODES.JACKPOT_DEPOSIT ? 'hidden' : ''
                      }`}
                    >
                      {props.searchParams?.deposit ? 'deposit' : 'withdraw'}{' '}
                      {props.searchParams?.crypto
            ...
            <div class='flex flex-col xl:flex-row'>
              ...
              >
                ...
                >
                  <p class='text-14 text-white font-medium font-Oswald'>$1.00</p>
                  <svg
                    width='16'
                    height='13'
                    viewBox='0 0 16 13'
                  ...
                  <div class='center gap-2'>
                    ...
                    <p class='text-14 text-white font-medium font-Oswald'>1 300</p>
                    <p class='text-14 text-yellow-ff font-medium font-Oswald'>(+30%)</p>
                  </div>
                </div>
                <div
                  class={`${
                    props.searchParams?.crypto || props.searchParams?.giftcard ? 'hidden' : 'flex'
                  ...
                  <div class='center flex-col w-full px-8 gap-3'>
                    ...
                    <div class='flex item-center justify-end sm:justify-center w-full'>
                      ...
                      <YellowGradientButton
                        isFullWidth={true}
                        callbackFn={() => {
                          props.searchParams?.deposit && props.searchParams?.giftcard
                            ? giftcardDeposit()
                            : props.searchParams?.deposit
                            ? deposit()
                            : props.pathname() == URL.GAMEMODES.JACKPOT_DEPOSIT
                ...
                <div class='center flex-col gap-2 flex-1 bg-white/[0.02] px-8 py-4'>
                  <div class='center flex-col w-full'>
                    <p class='text-14 text-gray-9a font-bold font-SpaceGrotesk capitalize'>
                      {cryptomethods[props.searchParams?.method]?.name} deposit address
                    </p>
                    <p class='text-[#646683]'>
                      You can send any amount of {cryptomethods[props.searchParams?.method]?.name}{' '}
                      to this address and receive equivalent in RustyLoot balance.
                    </p>
                    <div
                 ...
                 '
                   ...
                   linear-gradient(0deg, rgba(255, 255, 255, 0.02), rgba(255, 255, 255, 0.02));
                      ...
                      >
                        <p class='text-14 text-white font-normal'>{crypto()?.address}</p>
                        {crypto()?.address && (
                  ...
                  <p class='text-14 text-white font-semibold'>
                    Send any amount of {cryptomethods[props.searchParams?.method]?.name} to the
                    address above, and it'll be converted to site balance. 3 confirmation(s)
                    required.
                  </p>
                </div>
              </div>
                ...
                >
                  {(val) => (
                    <div class='relative hover rounded-4 overflow-hidden border border-white hover:border-yellow-ff duration-200 border-opacity-10'>
                      ...
                      >
                        <p class='text-12 text-white font-medium font-Oswald'>+30%</p>
                      </div>
                      <img
                        alt='zebras'
                        class=''
                        src={val.img}
                        onClick={() =>
                          window.open(`https://zebrasmarket.com/product?id=${val.link}`)
                        }
                      />
                    </div>
                  )}
