...
const CaseBattleJoinModal = (props) => {
  ...
  const modeColor = () =>
    getColorByModeAndCursed(props.game?.mode, props.game?.cursed);
  ...
  return (
    ...
    >
      ...
      <div
        class="rounded-xl flex flex-col absolute top-32 w-11/12 lg:w-[768px] max-h-[600px] overflow-x-scroll"
        style={{
          background:
      ...
      >
        <div class="border border-black/20 w-full p-6 lg:pl-[22px] lg:pr-8 lg:py-5 relative transition-all duration-100 ease-out grid grid-cols-7 grid-rows-2 lg:flex gap-[6px] lg:gap-0 justify-between items-center lg:h-[88px]">
          <div class="col-span-5 lg:col-auto uppercase text-white font-SpaceGrotesk text-20">
            join battle
          </div>
          <div
            class="col-span-5 col-start-1 row-start-2 lg:col-auto lg:row-auto w-max center px-5 py-3 border-white border-opacity-5 text-gray-9a border rounded-4 flex gap-1 items-center"
            classList={{
              ...
              {(value, index) => (
                <>
                  ...
                  {index() + 1 !== props.game?.playersQty && (
                    <>
                      {props.game?.cursed === 1 && (
                        <BattleCursedIcon additionClasses="w-3" />
                      )}
          ...
          <div
            onClick={props.handleClose}
            class="col-span-2 col-start-7 row-start-1 lg:col-auto lg:row-auto text-gray-9a w-10 h-10 flex items-center justify-center border rounded border-[#FFFFFF0A] cursor-pointer"
          >
            <svg
              width="10"
              height="10"
        ...
        <div
          class="relative flex flex-col items-center justify-center mb-4 lg:mb-8 px-6"
          style={{
            background:
              "linear-gradient(180deg,  rgba(217, 217, 217, 0) 70%, rgba(27,30,56,1 ) 100%)",
          }}
        >
          <div
            class="absolute w-full h-full z-[-1]"
            style={{
              "background-image": `url('${BodyVectorBackground}')`,
              opacity: 0.004,
            }}
          />
          <div class="w-full lg:w-max flex flex-col gap-4 my-4 lg:my-">
            <div class="flex flex-col items-center gap-2">
              <span class="font-SpaceGrotesk text-13 font-bold text-gray-a2">
                Choose your spot
              </span>
              <div
                class="border border-gray-600/50 rounded-6 flex items-center gap-2 lg:gap-2.5 px-[13.68px] lg:px-4 py-[9px] lg:py-2.5"
                style={{
                  background:
                    ...
                    <>
                      <div
                        onClick={() => {
                          if (
                            player !== null ||
                            player?.id === userObject.user?.id
                          )
                            return;
                          setSetup((prevState) => ({
                            ...prevState,
                            player_index:
                              setup().player_index === index() + 1
                                ? null
                                : index() + 1,
                          }));
                        }}
                        class={`cursor-pointer rounded-full flex items-center justify-center w-[42px] lg:w-12 h-[42px] lg:h-12 grow bg-blue-282 ${
                          !player &&
                          setup().player_index !== index() + 1 &&
                          "text-gray-9a"
                        } ${
                      ...
                      {index() + 1 !== props.game?.playersQty && (
                        <>
                          {props.game?.cursed === 1 && (
                            <BattleCursedIcon additionClasses="text-[#DAFD09] w-4 lg:w-5" />
                          )}
                          {props.game?.mode === "group" &&
                            props.game?.cursed !== 1 && (
                              <BattleGroupIcon additionClasses="text-[#5AC3FF] w-4 lg:w-5" />
                            )}
                          {(props.game?.mode === "royal" ||
                            props.game?.mode === "team") &&
                            props.game?.cursed !== 1 && (
                              <BattleRoyaleIcon additionClasses="w-4 lg:w-5 text-yellow-ffb" />
                            )}
                        </>
                      )}
                    </>
            ...
            >
              <div class="flex gap-2 items-center">
                ...
                {props.game?.fundBattle ? (
                  <div
                    class={
                      "rounded-2 absolute discond-green-border right-0.5 -top-1 -left-5 z-10 px-1 -rotate-[10deg] w-14 center text-green-3e font-Quicksand font-bold text-14"
                    }
                    style={{
                      background:
            ...
            <div class="flex flex-col min-w-full xll:min-w-[420px] fourk:min-w-[439px]">
              ...
              >
                <div class="grow px-3 lg:px-4 py-2 lg:py-3 flex gap-3 border border-white border-opacity-5 rounded-l h-full">
                  ...
                  <div class="flex flex-col font-SpaceGrotesk font-bold text-13">
                    ...
                    <span class="text-gray-a2 break-words text-13">
              ...
              {setup().borrowMoney === 1 && (
                ...
                >
                  ...
                  <div class='w-[240px] md:w-full md:scale-100 scale-[0.8] flex items-center justify-center'>
                  <RangePercentScale
                    value={setup().borrowPercent}
                    setter={(per) =>
                      setSetup((prev) => ({ ...prev, borrowPercent: per }))
                    }
                    maxPercent={80}
                    hexColor="#27F278"
                  />
                  ...
                  >
                    <p class="text-center font-SpaceGrotesk text-11 font-bold text-white">
                      ...
                      <span class="text-green-27">
                        {100 - Math.floor(setup().borrowPercent * 0.8)}%
                      </span>{" "}
                      of total win amount!
                    </p>
                  </div>
