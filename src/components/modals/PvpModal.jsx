  ...
  onMount(() => {
    setInterval(() => {
      if (game()?.status != "pending") {
        if (game()?.turn && !game()?.players[game()?.turn % game()?.playersAmount]?.bot) {
    ...
    >
      ...
      >
        <div
          ...
          style={{
            background: "linear-gradient(180deg, rgba(118, 124, 255, 0) 25%, rgba(118, 124, 255, 0.12) 50%, rgba(118, 124, 255, 0) 100%), linear-gradient(0deg, rgba(0, 0, 0, 0.24), rgba(0, 0, 0, 0.24)), linear-gradient(90.04deg, #1A1B30 0%, #191C35 75%)",
          }}
        >
          <div
          ...
          <div class="w-full flex flex-col gap-2">
            <div
              class="w-full flex flex-col-reverse llg:flex-row gap-10 xl:gap-20 overflow-y-scroll h-max max-h-[70vh]"
                      ...
                      <div
                        class={`w-full h-22 min-w-[368px] bg-dark-22 flex justify-between items-center px-6 py-3 relative bg-cover rounded-4 border-l-2 ${
                          (game()?.status == "started" &&
                            ...
                            ? "border-[#86FFB6]"
                            : game()?.status == "pending" || (game()?.status !== "ended" &&
                            val &&
                            game()?.turn % game()?.playersAmount !==
                              val?.index && !val?.eliminated)
                        ...
                        style={
                          ...
                          game()?.turn % game()?.playersAmount ==
                            val?.index)
                            ? {
                              background: 'linear-gradient(90deg, rgba(255, 180, 54, 0.08) 0%, rgba(255, 180, 54, 0) 93.23%)'
                            }
                          : (game()?.status == "ended" &&
                          game()?.winner?.player?.index == val.index)
                          ? {
                                background: 'linear-gradient(90deg, rgba(134, 255, 182, 0.08) 0%, rgba(255, 180, 54, 0) 93.23%)'
                              }
                              : game()?.status == "pending" || (game()?.status !== "ended" &&
                              val &&
                              game()?.turn % game()?.playersAmount !==
                                val?.index && !val?.eliminated)
                                ? {
                                  background: 'linear-gradient(90deg, rgba(154, 161, 223, 0.08) 0%, rgba(154, 161, 223, 0) 100%)'
                                }
                                : {
                                  background: 'linear-gradient(90deg, rgba(214, 51, 51, 0.08) 0%, rgba(154, 161, 223, 0) 100%)'
                                }
                        }
                      >
                        <div class="w-full flex items-center gap-3">
                            ...
                            ? "#86FFB6"
                              ...
                              isTransparentImage={!(game()?.status === "started" && (game()?.status == "started" &&
                              val &&
                              game()?.turn % game()?.playersAmount ==
                                val?.index) ||
                            (game()?.status == "ended" &&
                              game()?.winner?.player?.index == val.index))}
                            />
                            {val?.username ? <div class="flex flex-col gap-2">
                              <div
                            ...
                            >
                              ...
                              <div class={`flex gap-2 items-center font-SpaceGrotesk font-bold ${
                                (game()?.status == "started" &&
                                  val &&
                                  game()?.turn % game()?.playersAmount ==
                                    val?.index)
                            ...
                            ? <div class="mx-auto"><YellowGradientButton
                          isFullWidth={false}
                          callbackFn={callbot}
                        >
                          ...
                          : <div class="mx-auto"><YellowGradientButton
                            ...
                            >
                              <div class='flex gap-2 text-14 font-SpaceGrotesk font-bold text-yellow-ffb items-center'>
                                ...
                                <Coin width='5' />
                                <span class='text-gradient'>{game()?.value}</span>
                              </div>
                            </YellowGradientButton></div>}
              ...
              <div class="center gap-6 flex-col md:flex-row grow">
                <div class="relative flex flex-col gap-2 justify-center">
                  <div
                    ...
                    style={{
                      background: 'radial-gradient(50% 100% at 50% 0%, rgba(154, 158, 200, 0.24) 0%, rgba(154, 158, 200, 0) 100%)',
                      border: '1px solid',
                      'border-image-source': 'radial-gradient(36.46% 36.46% at 50% 100%, rgba(154, 158, 200, 0.36) 0%, rgba(154, 158, 200, 0) 100%)'          
                    }}
                  >
                  ...
                  <div
                    ...
                    style={{
                      background: 'radial-gradient(50% 100% at 50% 0%, rgba(255, 180, 54, 0.24) 0%, rgba(255, 180, 54, 0) 100%)',
                      border: '1px solid',
                      'border-image-source': 'radial-gradient(36.46% 36.46% at 50% 100%, rgba(255, 180, 54, 0.36) 0%, rgba(255, 180, 54, 0) 100%)'          
                    }}
                  >
                  ...
                  <div class="relative mb-4">
                    <div
                      class={`grid grid-cols-5 gap-4 w-max ${game()?.status == "ended" ? 'opacity-10' : ''}`}
                    >
                      <For
                        each={[
                    ...
                    {game()?.status == "ended" ? <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 center flex-col">
                      <UserGameAvatar
                      ...
                      />
                      <div class="text-gradient-green-secondary font-SpaceGrotesk text-28 font-bold">Winner</div>
