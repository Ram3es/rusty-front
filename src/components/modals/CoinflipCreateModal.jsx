...
const CoinflipCreateModal = (props) => {
    ...
    createEffect(() => {
        if(props.pathname() == URL.GAMEMODES.COINFLIP_CREATE || props.pathname() == URL.GAMEMODES.COINFLIP_JOIN) {
        ...
        if(selectedItems.length > 0 && selectedItems.length < 30) {
            ...
            socket.emit("coinflip:deposit", {
                ...
                if(!data.error && data?.data?.link) {
                    ...
                    setUserObject("trades", (prev) => [...prev, {link: data.data.link, type: "deposit", expires: Date.now() + 1000 * 60 * 5}]);
                }
            });
        }
    }
    ...
    const refresh = () => {
        ...
        socket.emit("steam:inventory:refresh", {}, (data) => {
            ...
            if (!data.error && (props.searchParams?.deposit && props.searchParams?.items) || props.pathname() == URL.GAMEMODES.COINFLIP_CREATE) {
                setItems(data.data.items)
    ...
    const autoSelect = () => {
        ...
        const temp = {
            items: items().filter((item) => item.price <= limits.upper).sort((a,b) => (b.price - a.price)),
            value: 0,
            selectedIds: []
        }
    ...
    return (
        <Modal open={() => {
            ...
            <NavLink class="w-full h-full absolute left-0 cursor-default top-0" onClick={() => setItems([])} href={URL.GAMEMODES.COINFLIP} />
            <div class={`flex flex-col z-10 relative w-11/12 rounded transition-all transform -translate-y-1/4 ${props.open || props.pathname() == URL.GAMEMODES.COINFLIP_CREATE || props.pathname() == URL.GAMEMODES.COINFLIP_JOIN ? "-translate-y-0" : ""} duration-100 ease-out`} style={{
                "max-width": "1496px",
                "max-height": "90vh",
            }}>
                <div
                ...
                >
                    ...
                    >
                        <div class="w-full flex justify-between items-center">
                            <p class="text-20 text-white font-bold font-SpaceGrotesk uppercase truncate">{props.pathname() == URL.GAMEMODES.COINFLIP_JOIN ? i18n.t('coinflip.Join coinflip') : i18n.t('coinflip.Create')}</p>
                        </div>
                    ...
                    <div class="flex w-full flex-col xl:flex-row min-h-full justify-between">
                        <div class={`w-full px-8 flex flex-col grow gap-8 py-4`}>
                                ...
                                <div class="center gap-2">
                                    <div class='balance-bg rounded-4 flex items-center drop-shadow-dark'>
                                        <div class='bg-black bg-opacity-10 rounded-4 h-9 flex m-0.5 '>
                                            <div
                                            class=' w-full h-full px-3 bg-cover py-1 text-16 text-gray-e0 rounded-4 flex gap-2 items-center font-Lato font-bold'
                                            >
                                            <Coin width="6" />
                                            <p class="text-gradient text-16 font-medium font-Oswald">
                                                {items().reduce((prev, cur) => prev += cur.price, 0).toLocaleString()}
                                            </p>
                                    ...
                                    <div class={`bg-gray-button-gradient h-10 w-12 p-2 rounded-4 border border-white/10 ${props.searchParams?.deposit || props.pathname() == URL.GAMEMODES.COINFLIP_CREATE || props.pathname() == URL.GAMEMODES.COINFLIP_JOIN ? "center" : "hidden"}`} onClick={refresh}>
                                        <svg ref={spinArrow} width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <Items isCoinflipPage={true} items={() => {
                                return items().filter((item) => (!search() || (String(item.name).toLowerCase().includes(String(search()).toLowerCase()))))
                            }} descending={descending} toggle={toggle} activeItems={activeItems} />
                        </div>
                        <div
                            ...
                            >
                                 <div class="flex flex-col gap-2 pt-4 max-h-full overflow-y-scroll border-b border-black/10 h-full px-8 ">
                                    ...
                                    >
                                        ...
                                        <PotentialDropItem skin={item} isHorisontal={true} />
                                    </div>)}
                                    </For>
                                </div>
                            </div>
                            <div class="w-full px-8 py-4 flex flex-row md:flex-col justify-center md:justify-between items-center gap-3">
                                <div class={`${props.pathname() == URL.GAMEMODES.COINFLIP_JOIN ? "flex" : "hidden"} text-gray-a2 pt-0.5 text-12 px-3 h-6 bg-gray-a2/10 border border-gray-a2/20 border-dashed rounded-4`}>
                                    {Math.round(props.searchParams.value * 0.98)} - {Math.round(props.searchParams.value * 1.02)}
                                </div>
                                <div class={`${props.pathname() == URL.GAMEMODES.COINFLIP_JOIN ? "hidden" : "center"} gap-3`}>
                                    <div
                                        class={`p-1 rounded-full ${side() == 1 ? "border border-0.5 border-yellow-ffb" : ""}`}
                                        onClick={() => {
                                            setSide(1);
                                        }}
                                    >
                                    ...
                                    <div
                                        class={`p-1 rounded-full ${side() == 2 ? "border border-0.5 border-yellow-ffb" : ""}`}
                                        onClick={() => {
                                            setSide(2);
                                        }}
                                    >
                                ...
                                >
                                    <div class='flex capitalize gap-2 text-14 font-SpaceGrotesk font-bold text-yellow-ffb items-center'>
                                        {props.pathname() == URL.GAMEMODES.COINFLIP_JOIN ? "Join Coinflip" : "Create Coinflip"}
                                    </div>
                                </YellowGradientButton>
                                <div
                                    class={`w-full bg-gray-button-gradient font-SpaceGrotesk font-bold h-10 p-2 rounded-4 border text-gray-9a border-white/10 ${props.pathname() == URL.GAMEMODES.COINFLIP_JOIN ? "center" : "hidden"}`}
                                    onClick={autoSelect}
                                >
                                    Auto-Select
                                </div>
