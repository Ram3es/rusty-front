...
const CaseGradientButton = (props) => {
  return (
    <div class={`rounded-4 h-10 z-10 flex items-center justify-center ${props.classList ?? 'p-[1px]' } ${props.additionalClass ?? ''}`}
     style={{
      ...
      <div class={`${!props.selected && props.toggle && "bg-[#19171f]"} h-full rounded-4 w-full`}>
        <div
          class={`h-full rounded-4 px-3 lg:px-4 ${
            ...
            : 'mint-button-gradient' } rounded-4 flex center cursor-pointer ${props.isFullWidth ? 'w-full' : 'w-full lg:w-max'}`}
          onClick={() => {
