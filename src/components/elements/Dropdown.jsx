    ...
    <div class="flex gap-2 items-center text-yellow-ffb">
      ...
      {props.activeName.includes("-") ? (
        () => {
          ...
          return (
            <>
              <GoldText text={min} size="14" />
              <GoldText text=" - " />
              <GoldText text={max} size="14" />
            </>
          );
        }
      ) : (
...
const Dropdown = (props) => {
  ...
  return (
    <div
      class="relative"
      classList={{"w-full": props.isFullWidth}}
      ref={dropdownWrapper}
    >
      <GrayGradientButton
        callbackFn={() => {
          if (props.toggleClick) {
            props.submitItem(props.activeName === "ASC" ? "DESC" : "ASC");
          } else {
            setIsOpen((prev) => !prev);
          }
        }}
      ...
      <ul
        ...
        style={{
          background:
            "radial-gradient(58.03% 60.37% at 50% 29.27%, rgba(118, 124, 255, 0.07) 0%, rgba(118, 124, 255, 0) 100%), linear-gradient(0deg, rgba(255, 255, 255, 0.12), rgba(255, 255, 255, 0.12)), radial-gradient(100% 275.07% at 100% 0%, rgba(29, 35, 82, 1) 0%, rgba(29, 31, 48, 1) 100%)",
        }}
        tabindex="-1"
        role="listbox"
        aria-labelledby="listbox-label"
